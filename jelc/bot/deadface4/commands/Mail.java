...
import bot.deadface4.commands.mail.*;
		...
		if(args[0].equalsIgnoreCase("mail")&&(args.length>1)){
			if(args[1].equalsIgnoreCase("send")&&(args.length>2)){				MailSender mail=new MailSender(reply);
				...
				for(int i=2;i<args.length&&ok;i++){
					...
					if(account.canSendTO(reply.getName())){
						mail.addName(args[i]);
					}
					else{
						ok=false;
				...
				if(ok){
					reply.getAccount().setSession(mail);
				}
				else{
				...
				reply.getAccount().setSession(new MailSystem(reply));
				return true;
			}
