...
PlayerList defaultList;
	public Online(PlayersOnline on, SeenList2 seen2,MyBot4 bot){
		this.online=on;
		this.seen=seen2;
		this.bot=bot;
		buildList();
	...
	public boolean process(Replyer reply, String[] args){
    	if(args[0].equals("online")){
    		...
    		if(args.length==1){
    			...
    			Misc.ouputList(reply,defaultList.getName()+": ",online.getOnline(defaultList.getList()).listIterator());
    		}
    			...
    			if((args[1].equals("list"))||(args[1].equals("lists"))){
    				...
    				for(Iterator itr=views.iterator();itr.hasNext();){
    					...
    					if(tmp.getName().equals(args[1])){
    						...
    						Misc.ouputList(reply,tmp.getName()+": ",online.getOnline(tmp.getList()).listIterator());
    						found=true;
    					}
    				}
    				if(!found){
