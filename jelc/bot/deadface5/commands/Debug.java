...
MyBot bot;
	...
	public boolean process(Replyer reply, String[] args) {
		if(on){
			System.out.println("name: "+reply.getAccount().getName()+" args: "+args.length);
		...
		else if(args[0].equalsIgnoreCase("accountdump")&&(args.length==2)){
			Account a=AccountManager.getAccountManger().getAccount(args[1]);
			reply.reply("Account:"+a.dump());
			return true;
		}
		return false;
	}
