...
boolean sentPing=false;
		...
		if(args[0].equalsIgnoreCase("bot")&&(reply.getAccount().isBot()||reply.getAccount().isAdmin())){
			...
			if(args.length==2){
				if(args[1].equals("active")){
					...
					//bot.reply(name,"bot gtpm "+bot.guild.,MyBot.PM);
					return true;
				}
			...
			if(!active.contains(reply.getAccount().getName())){
				active.add(reply.getAccount().getName());
				reply.reply("bot active");
			}
			pending.remove(reply.getAccount().getName());
	...
	public void onMinute(int minute) {
		...
		if((r.nextInt()%20)==0){
			for(Iterator itr=botList.getList().iterator();itr.hasNext();){
				bot.reply(itr.next().toString(),"bot active",MyBot.PM);
			}
		}*/
	}
