...
File file =new File("guild.txt");*/
	...
	public boolean process(Replyer reply, String[] args) {
		if(reply.getAccount().isSameGuild()){
			if(args[0].equalsIgnoreCase("#gm")){
				...
				for(Iterator itr=Config.getConfig().getPassthrough().iterator();itr.hasNext();){
					...
					if(args[1].equalsIgnoreCase(passthrough)){
						...
						Channel c=BackendManager.getInstance().getChannel("el:gm");
						System.out.println("CHANEEL "+c+"  "+tmp);
						c.send(tmp);
						return true;
					}
				...
				BackendManager.getInstance().getChannel("el:gm").send(tmp);
			}
				...
				if(reply.getAccount().isSameGuild()){
					...
					local.send("#accept "+reply.getAccount().getName());
					/*if(isAdmin(name)){
						chat("#change_rank "+person+" 18");
					if}
					else{*/
					local.send("#change_rank "+reply.getAccount().getName()+" 5");					
					//}
					local.send("#gm welcome "+reply.getAccount().getName()+" back into the guild");
				}
				...
				if(reply.getAccount().isSameGuild()){
					...
					local.send("#remove "+reply.getAccount().getName());
					local.send("#gm "+reply.getAccount().getName()+" has been removed from the guild");
				}
			}
			...
			/*else if((args.length==2)&&args[1].equalsIgnoreCase("gmRedirect")){
	...
	public void onGm(String person, String message) {
		if(gmTo!=null){
			...
			Channel to=BackendManager.getInstance().getChannel("el:"+gmTo);
			to.send("#GM from "+person+": "+message);
