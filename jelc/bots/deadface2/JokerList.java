...
MyBot2 bot;
	...
	public boolean process(String name, String[] args, int type) {
		if(args[0].equalsIgnoreCase("joker")){
			...
			if(i.hasNext()){
				bot.outputList(name,type,list.size()+" people have found the joker (when i've been online) the top 10 are: ",i);
			}
			else{
				bot.reply(name,"no one has found him yet",type);
			}
			return true;
		}
		return false;
	...
	public void sendHelp(String name, int type) {
		bot.reply(name,"joker         - displays top 10 people to find the joker(that i've seen)",type);
	}
